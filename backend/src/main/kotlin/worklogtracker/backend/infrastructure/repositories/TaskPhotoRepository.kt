...
class TaskPhotoRepository : TaskPhotoRepositoryInterface {
    override suspend fun findById(id: TaskPhotoId): TaskPhotoEntity? =
        transaction {
            TaskPhotoTable
                .selectAll()
    ...
    override suspend fun save(photo: TaskPhotoEntity): TaskPhotoEntity =
        transaction {
            val now = LocalDateTime.now().toKotlinLocalDateTime()
            val id =
                TaskPhotoTable.insert {
    ...
    override suspend fun findByTask(taskId: TaskId): List<TaskPhotoEntity> =
        transaction {
            TaskPhotoTable
                .selectAll()
