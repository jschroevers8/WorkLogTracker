...
class TaskAssignmentRepository : TaskAssignmentRepositoryInterface {
    override suspend fun findById(id: TaskAssignmentId): TaskAssignmentEntity? =
        transaction {
            TaskAssignmentTable
                .selectAll()
    ...
    override suspend fun save(assignment: TaskAssignmentEntity): TaskAssignmentEntity =
        transaction {
            val now = LocalDateTime.now().toKotlinLocalDateTime()
            val id =
                TaskAssignmentTable.insert {
    ...
    override suspend fun update(assignment: TaskAssignmentEntity): Boolean =
        transaction {
    ...
    override suspend fun findByTask(taskId: TaskId): List<TaskAssignmentEntity> =
        transaction {
            TaskAssignmentTable
                .selectAll()
    ...
    override suspend fun findByUser(userId: UserId): List<TaskAssignmentEntity> =
        transaction {
            TaskAssignmentTable
                .selectAll()
